...
def render() -> None:
    ...
    st.markdown('<p class="kid-section-title">選擇服裝</p>', unsafe_allow_html=True)
    for row_start in range(0, len(outfits), 3):
        ...
        for index, outfit in enumerate(outfits[row_start : row_start + 3]):
            with cols[index]:
                equipped = outfit.get("outfit_id") == child.get("selected_outfit")
                _render_outfit_card(outfit, owned=True, equipped=equipped)
                if st.button(
                    "已裝備" if equipped else "裝備",
                    key=f"equip_outfit_{outfit['outfit_id']}",
                    disabled=equipped,
                    use_container_width=True,
                ):
                    try:
...
def _render_outfit_card(outfit: dict, *, owned: bool, equipped: bool = False) -> None:
    ...
    buff_label = str(buff.get("buff_label") or "本遊戲沒有額外助力")
    buff_description = str(buff.get("buff_description") or "這件裝備目前只作為外觀展示。")
    ...
    st.markdown(
        ...
        <div class="outfit-card{equipped_class}">
            ...
            <p class="gear-buff-line">{escape(buff_label)}</p>
            <p>{escape(buff_description)}</p>
