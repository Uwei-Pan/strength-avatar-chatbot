...
這是一個 Streamlit + MySQL 的兒少優勢探索 MVP。孩子登入後可以和智慧小幫手聊天、寫心情日記、完成任務、玩遊戲、累積 tokens，並把成長亮點保存到資料庫。
...
COUNSELOR_TASK_REVIEW_PASSWORD=change_me
```
...
7. 在 Render Environment Variables 設定：
   ...
   - `GEMINI_MODEL`
   - `GEMINI_FALLBACK_MODELS`
   - `COUNSELOR_TASK_REVIEW_PASSWORD`
...
GEMINI_MODEL = "gemini-2.5-flash"
GEMINI_FALLBACK_MODELS = ""
COUNSELOR_TASK_REVIEW_PASSWORD = "your-review-password"
```
